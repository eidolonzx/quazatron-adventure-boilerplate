...
## 4. Создаём словари
 ...
 это создание словарей. В файлах-словарях находятся все слова, которые понимает игра. В QA таких словаря три: файл `verbs.js` с глаголами, `objects.js` с существительными и `adjectives.js` с прилагательными.
...
const initialItemPlaces = {
  0: 1, // Перчатки (id = 0) находятся в локации 1
};
```
...
В нашей мини-игре помимо стандартных "взять", "положить" и "осмотреть" мы реализуем ещё три команды:

1. Игрок сможет одеть перчатки, которые найдёт в локации 1.
2. Если перчатки одеты, игрок сможет их снять.
3. Игрок сможет нажать на кнопку, которую он обнаружит на пульте в локации 2.

Команды "НАДЕТЬ", "СНЯТЬ" и "НАЖАТЬ" уже есть среди глаголов по-умолчанию, и нам останется лишь прописать код внутри их методов.
